...
def update_high_score(high_score, score):
    if high_score < score:
        high_score = score
    with open(os.getcwd() + 'high_score.txt','w') as file:
...
def read_high_score():
    with open(os.getcwd() + "high_score.txt", 'r') as file:
        return file.read()


''' This function return the skin the user have. '''
def read_skin():
    with open(os.getcwd() + 'skins.txt', 'r') as file:
        return file.read()
...
def buy_skin(skin):
    skins = read_skin()
    with open(os.getcwd() + 'skins.txt','w') as file:
        file.write(skins + '\n' + str(skin))
